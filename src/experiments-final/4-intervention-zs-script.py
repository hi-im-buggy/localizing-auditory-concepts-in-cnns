...
parser.add_argument('--partition_idx', type=int)
...
# %%
pdf_path = f'/scratch/pratyaksh.g/{probing_dataset.path_name}/cluster-plots/{layer_name}/partition-{args.partition_idx}/'
os.makedirs(pdf_path, exist_ok=True)

for file in os.listdir(pdf_path):
    if file.endswith(f'cluster-{args.cluster_idx}.pdf'):
...
cluster_mask = (clusters[:, args.partition_idx] == args.cluster_idx).bool()
if args.invert_mask:
...
for run in range(args.repeats):
    y_preds, y_labels = [], []
    for batch in tqdm(loader, desc=f"Run {run + 1}/{args.repeats}"):
...
expt_id = f"partition-{args.partition_idx}-cluster-{args.cluster_idx}-{args.treatment}-invert_mask={str(args.invert_mask)}.csv"
